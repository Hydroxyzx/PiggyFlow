...
public class StatsFragment extends Fragment {
    ...
    private String rangeLabel(long start, long end) {
        if (mode == WEEK) {
            SimpleDateFormat f = new SimpleDateFormat("dd MMM", ID);
            SimpleDateFormat fy = new SimpleDateFormat("dd MMM yyyy", ID);
            return f.format(new Date(start)) + " – " + fy.format(new Date(end));
        } else if (mode == MONTH) {
            return new SimpleDateFormat("MMMM yyyy", ID).format(new Date(start));
        } else if (mode == YEAR) {
            return new SimpleDateFormat("yyyy", ID).format(new Date(start));
        } else {
            SimpleDateFormat f = new SimpleDateFormat("dd MMM yyyy", ID);
    ...
    private void render(List<Txn> list) {
        ...
        if (byCat.isEmpty()) {
            ...
            empty.setText("Belum ada pengeluaran pada periode ini.");
