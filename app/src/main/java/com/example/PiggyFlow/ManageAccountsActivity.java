...
public class ManageAccountsActivity extends AppCompatActivity {
    ...
    private class Adapter extends RecyclerView.Adapter<Adapter.VH> {
        ...
        @Override public void onBindViewHolder(VH h, int i) {
            ...
            h.tvSub.setText(a.type + " — Saldo awal: " + String.format("%,.2f", a.openingBalance));
            h.btnDelete.setOnClickListener(v -> confirmDelete(a));
            // long-press tetap berfungsi sebagai cara alternatif
            h.itemView.setOnLongClickListener(v -> { confirmDelete(a); return true; });
        }
    ...
    private void confirmDelete(Account a) {
        new AlertDialog.Builder(this)
                .setTitle("Hapus akun?")
                .setMessage("\"" + a.name + "\" akan dihapus. Transaksi yang sudah terhubung dengan akun ini akan tetap ada tapi tidak menampilkan nama akun lagi.")
                .setNegativeButton("Batal", null)
                .setPositiveButton("Hapus", (d, w) ->
                        Executors.newSingleThreadExecutor().execute(() ->
                                AppDatabase.get(this).accountDao().delete(a)))
                .show();
    }
}
